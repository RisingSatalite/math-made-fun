...
export default function SurfaceAreaCube() {
    ...
    function calculateSurfaceArea() {
        setSurfaceArea(((length*width)+(height*width)+(length*height)) * 2);
    }
    ...
    return (
        ...
        <input onChange={(e) => setHeight(Number(e.target.value))} type="number" placeholder="Length of one side (L)" className="border p-2 rounded w-64" />
        <input onChange={(e) => setWidth(Number(e.target.value))} type="number" placeholder="Length of one side (L)" className="border p-2 rounded w-64" />
